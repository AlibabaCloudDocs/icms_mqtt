...
在 RAM 中，权限策略是用[语法结构](../../../../cn.zh-CN/权限策略管理/权限策略语言/权限策略语法和结构.md#)描述的一组权限的集合，可以精确地描述被授权的 Resource（资源集）、Action（操作集）以及授权条件。微消息队列 MQTT 有以下两类 RAM 的权限策略：
...
-   [通过RAM限制用户的登录IP地址](../../../../cn.zh-CN/教程/通过RAM限制用户的登录IP地址.md#)
